...
- **Auto-update / release-checking is explicitly out of scope for this SDK** (gaps #1–#2, Tamga API protocol specification §12). `GET /releases/actions/upgrade` crashes at runtime (missing DB tables/columns) and there's no working artifact-download endpoint even once that's fixed. Do not add a `CheckForUpdateAsync`-style method to `Client.cs` — there's nothing working to call.
