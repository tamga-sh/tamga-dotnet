...
**Publishing method — `NUGET_API_KEY` secret is the deliberate v0.1 default, not a placeholder to replace immediately.** NuGet.org's Trusted Publishing/OIDC-based publishing is newer and not yet confirmed stable enough across `dotnet nuget push` tooling specifically to be the primary path for this repo. The repository secret is boring, well-documented, and works today. Revisit Trusted Publishing once it's a well-trodden path for `dotnet nuget push` workflows specifically (not just `nuget push` or other toolchains) — this is a living decision, not a permanent one.

**Known blocker — `NUGET_API_KEY` is not actually configured yet.** `.github/workflows/release.yml`'s `publish` job reads `secrets.NUGET_API_KEY`, but *setting* a GitHub Actions repository secret requires GitHub repo-admin access (Settings → Secrets and variables → Actions) — there is no file in this repository that can express it, and no local/CLI-only workflow can create it without `gh secret set` credentials tied to a real, admin-authorized session against the `tamga-sh/tamga-dotnet` repo. This is the one remaining genuinely external, non-autonomous step blocking a real `0.1.0` publish: generate an API key scoped to the `Tamga.Sdk` package at nuget.org, then a repo admin runs `gh secret set NUGET_API_KEY --repo tamga-sh/tamga-dotnet` (or the GitHub UI equivalent). Everything else in the release pipeline (release-please config, MinVer wiring, the `publish` job itself) is in place and was verified locally via `dotnet pack` against a local `v0.1.0-alpha` tag — see `docs/plans/tamga-dotnet.plan.md` §M's checkbox note.
