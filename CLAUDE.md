...
- **The machine heartbeat window IS policy-driven; the 600s figure is only a fallback** (corrects the old gap #8 claim). `Policy::effective_heartbeat_duration_secs` (`tamga-api/src/features/policies/model.rs:262-264`) returns `policy.heartbeat_duration` when set and `600` only when it is null, and the culler measures against `COALESCE(p.heartbeat_duration, 600)` (`tamga-api/src/workers/machine_jobs.rs:213`). Do NOT write "hardcoded 600s", "fixed 600s", or "not driven by `policy.heartbeat_duration`" anywhere. `HeartbeatScheduler.ServerHeartbeatWindowSeconds` keeps its name for source compatibility but documents itself as the DEFAULT. **The other half must always be stated with it:** this SDK has no policy or machine getter, so the scheduler cannot read the effective window and `DefaultInterval` stays ~1/3 of the 600s fallback — on a shorter-window policy the caller must pass their own interval. Do not "fix" this by adding a policy getter or a policy-aware scheduler; that is a feature, not a doc correction.
...
- **`DEAD` does NOT mean the row was culled, and `HeartbeatScheduler` must never stop pinging on it.** Two independent server facts: (1) `Machine::heartbeat_status*` derives the field from `last_heartbeat_at` vs the window and never consults `require_heartbeat`; (2) the cull job early-returns when `!policy.require_heartbeat`, and its claim query requires `AND p.require_heartbeat` — a column that `DEFAULT FALSE`. So on a default policy **nothing is ever culled** and a machine reports `DEAD` forever with its row and its seat still present. A ping to a `DEAD` machine succeeds and revives it (bare `SET last_heartbeat_at = NOW()`, no resurrection gate). The ping loop therefore raises the `Dead` event as a notification and **continues** — breaking, returning, or `continue`-ing past the next tick there is a real bug, not a nicety (it is exactly the bug tamga-python shipped), and `HeartbeatSchedulerTests` has a regression test pinning three consecutive `DEAD` responses. The only row-is-gone signal is a **`404 NOT_FOUND` from the ping**, surfaced on `Faulted` as `TamgaNotFoundException`; re-activation hangs off that. Do not write docs, samples, or XML comments that say `DEAD` means culled/deleted/removed.
